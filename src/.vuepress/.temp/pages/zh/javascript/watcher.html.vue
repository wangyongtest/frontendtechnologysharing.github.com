<template><div><!-- more -->
<h2 id="发布-订阅模式" tabindex="-1"><a class="header-anchor" href="#发布-订阅模式" aria-hidden="true">#</a> 发布-订阅模式</h2>
<p>存在一个“信号中心”，某个任务完成，就向信号中心，发布（publish）一个信号，其他任务向信息中心订阅（subscribe）这个信号，从而知道自己什么时候可以开始执行，即：发布-订阅模式。</p>
<ul>
<li>
<p>示例</p>
<div class="language-typescript line-numbers-mode" data-ext="ts"><pre v-pre class="language-typescript"><code><span class="token operator">&lt;</span>script<span class="token operator">></span>
...
<p>当事件发生时，<strong>观察者</strong>(订阅者，watcher)执行update方法，完成相应的操作；被观察的<strong>目标</strong>(发布者，Dep)使用数组Subs来存储所有的观察者， 也可以使用
addSub()添加新的观察者，当有事件发生时，调研所有时间的update()方法</p>
<ul>
<li>
<p>示例</p>
<div class="language-typescript line-numbers-mode" data-ext="ts"><pre v-pre class="language-typescript"><code><span class="token operator">&lt;</span>script<span class="token operator">></span>
...
<ul>
<li>发布-订阅模式：由一个统一调度中心调用，发布者和订阅者之间不需要知道对方的存在</li>
<li>观察者模式：由具体的目标调度，如，某一个事件被触发时，Dep就会调用数组中保存的和这个事件有关系的观察者的方法，因此，观察者模式中的订阅者与发布者之间存在依赖关系</li>
</ul>
<!-- @bytemd/react --></div></template>
